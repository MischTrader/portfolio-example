# Welcome to My Data Science Portfolio

This portfolio contains a summary of my work, including machine learning exercises and critical reflections.

## Table of Contents

1. **[Hyperparameter gridsearch]**
   - [Opdracht 1 Summary](1-hypertuning-gridsearch/summary.md)
2. **[Hyperparameter mlflow]**
   - [Opdracht 2 Summary](2-hypertuning-mlflow/summary.md)
3. **[Hyperparameter rnn]**
   - [Opdracht 3 Summary](3-hypertuning-rnn/summary.md)
4. **[Hyperparameter ray]**
   - [Opdracht 4 Summary](4-hypertuning-ray/summary.md)
5. **[Hackathon Model: Kadaster Rechtsfeiten]**
   - [Opdracht 5 Hackathon Model]([4-hypertuning-ray/summary.md](https://github.com/MaxNollet/mads-2025-hackathon/tree/main/src/akte_classifier/models))
6. **[Deployment: Nijmeegse Straattaal Generator]**
   - Live NLP-applicatie gedeployed op SURF
   - Dockerized FastAPI backend met HTML frontend
   - [Bekijk deployment details →](5-deployment/summary.md)
   - [Live applicatie → http://145.38.194.28]
7. **[Ethical Reflection: The Cuddlefish Dilemma]**
   - A critical analysis of the ethical implications of AI-driven camouflage.
